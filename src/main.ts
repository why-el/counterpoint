...
try{if(params.get('no-graphics')==='1')throw new Error('WebGL disabled for fallback review');sculpture=createSculpture(canvas,params.get('single')==='1'?1:8,Number(params.get('lighting'))||0,quality,()=>{dirty=true;});quality=sculpture.stats().quality;sculpture.setCamera(imported.c);}
...
NOTES.forEach((note,i)=>{const b=document.createElement('button');b.className='note';b.dataset.note=String(i);b.setAttribute('aria-label',`${note.name} note`);b.setAttribute('aria-pressed',String(imported.s.mask[i]));b.innerHTML=`<span class="pip" aria-hidden="true"></span><span>${note.name}</span>`;b.onclick=()=>toggleNote(i);notes.append(b);});
function updateUI(){const t=transport.read();const desired=score.desired();const active=score.at(t);
 ...
 $('sound-label').textContent=audio.starting?'Starting…':audio.enabled?'Sound on':'Sound off';$<HTMLButtonElement>('sound').disabled=audio.starting;$('sound').setAttribute('aria-pressed',String(audio.enabled));
 $('play-state').classList.toggle('held',!transport.playing);$('play-state').querySelector('span')!.textContent=scrubbing?'ADJUSTING TIME':transport.playing?'PLAYING':'PAUSED';
 ...
 if(pendingAt!==null&&t>=pendingAt){pendingAt=null;$('queue-status').textContent='Moving marbles finish their routes.';}
}
function pause(){transport.pause();audio.cancel();dirty=true;updateUI();}
function scheduleAudio(){audio.schedule(score,transport.read(),transport.playing&&!scrubbing&&!hidden);}
function play(){if(document.hidden)return;transport.play();audio.cancel();scheduleAudio();dirty=true;updateUI();}
...
function toggleNote(i:number){const desired=score.desired();desired.mask[i]=!desired.mask[i];pendingAt=score.queue(desired,transport.read());audio.cancel();scheduleAudio();$('queue-status').textContent='Changes are queued.';announce(`${NOTES[i].name} ${desired.mask[i]?'enabled':'disabled'}. Moving marbles finish their routes.`);dirty=true;updateUI();}
$('play').onclick=()=>transport.playing?pause():play();
$('sound').onclick=async()=>{if(audio.enabled&&audio.context?.state==='running'){transport.useClock(()=>performance.now()/1000);audio.disable();announce('Sound off');}else{try{const activation=audio.enable();updateUI();await activation;transport.useClock(()=>audio.context!.currentTime);audio.context!.onstatechange=()=>{if(audio.enabled&&audio.context?.state!=='running'){pause();announce('Audio paused. Press Sound to reconnect.');}};scheduleAudio();announce(transport.playing?'Sound on':'Sound on. Press Play to hear the notes.');}catch{audio.disable();announce('Sound could not start. Press Sound to try again.');}}updateUI();};
...
$('variation').onclick=()=>{const c=score.desired();c.variation=(c.variation+1)%3;pendingAt=score.queue(c,transport.read());audio.cancel();scheduleAudio();$('queue-status').textContent='Changes are queued.';announce(`${VARIATIONS[c.variation]} selected. Moving marbles finish their routes.`);dirty=true;updateUI();};
...
$('copy-link').onclick=async()=>{try{await navigator.clipboard.writeText($<HTMLInputElement>('share-url').value);$<HTMLDialogElement>('share-dialog').close();announce('Arrangement link copied');}catch{const field=$<HTMLInputElement>('share-url');field.focus();field.select();announce('Link selected. Copy it with your keyboard.');}};
...
const input=attachInput(canvas,s,{time:()=>transport.read(),begin:()=>{pause();scrubbing=true;updateUI();},seek,end:(forward)=>{scrubbing=false;if(forward)play();else{pause();announce('Paused. Press Play to continue.');}},note:toggleNote,changed:()=>{dirty=true;}});
...
document.addEventListener('visibilitychange',()=>{hidden=document.hidden;if(hidden){input.cancel();pause();cancelAnimationFrame(raf);}else{lastFrame=performance.now();dirty=true;raf=requestAnimationFrame(frame);announce('Paused while the tab was hidden. Press Play to continue.');}});
canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();pause();cancelAnimationFrame(raf);fallback('Graphics were interrupted. Try again to reload.');});
canvas.addEventListener('webglcontextrestored',()=>{$('fallback').hidden=true;dirty=true;lastFrame=performance.now();raf=requestAnimationFrame(frame);announce('Graphics restored. Press Play to continue.');});
...
window.setInterval(scheduleAudio,40);
