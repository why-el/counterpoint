...
export function createSculpture(canvas:HTMLCanvasElement,count=8,alternate=0,initialQuality='auto') {
 ...
 function project(p:THREE.Vector3){camera.updateMatrixWorld(true);const v=p.clone().project(camera);return {x:(v.x+1)*width/2,y:(1-v.y)*height/2};}
