...
test('real controls, deterministic reverse, sound, share, and camera',async({page,browserName},info)=>{
 ...
 await page.locator('#parts').click();await page.getByRole('button',{name:'D3 note',exact:true}).click();await expect(page.getByRole('button',{name:'D3 note',exact:true})).toHaveAttribute('aria-pressed','false');expect((await api(page).state()).score.mask[0]).toBe(false);
...
test('resilience, bounded URLs, offline operation and context restoration',async({page,browserName})=>{
 ...
 await page.context().setOffline(true);await page.locator('#parts').click();await page.getByRole('button',{name:'F4 note',exact:true}).click();expect((await api(page).state()).score.mask[4]).toBe(false);await page.context().setOffline(false);
